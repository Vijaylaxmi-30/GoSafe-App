...
class _MapScreenState extends State<MapScreen> {
      ...
      if (response.statusCode == 200) {
        ...
        if (routes.isNotEmpty) {
          ...
          setState(() {
            ...
            // logic for  duration
            // logic for  duration
            final duration = alternative['legs'][0]['duration']; // Duration in seconds
            final hours = (duration / 3600).floor(); // Get hours
            final minutes = ((duration % 3600) / 60).floor(); // Get remaining minutes
            String formattedDuration;
            if (hours > 0) {
            ...
            print("Duration alt: $formattedDuration");
            time2=formattedDuration;

            //logic for distance
            final distance = alternative['legs'][0]['distance']; // Distance in meters
            String formattedDistance;
            if (distance >= 1000) {
              formattedDistance = "${(distance / 1000).toStringAsFixed(2)} km"; // Convert to km
            } else {
            ...
            print("Distance alt: $formattedDistance");



            //logic for distance
            var altdistance = alternative['legs'][0]['distance']; // Distance in meters
            String altformattedDistance;
            if (altdistance >= 1000) {
              altformattedDistance = "${(altdistance / 1000).toStringAsFixed(2)} km"; // Convert to km
            } else {
              altformattedDistance = "${altdistance.toStringAsFixed(0)} meters"; // Show in meters
            }
            print("alt Distance: $altformattedDistance");
            dist2=altformattedDistance;

            setState(() {
              altdistance = formattedDistance;
            });
 ...
 // If score > 1, route is unsafe


  void _startNavigation(BuildContext context, List<LatLng> route, String routeType) {
    Navigator.push(
      context,
      MaterialPageRoute(
    ...
    print("Navigating using $routeType route");
  }
            ...
            Padding(
              ...
              child: Text(
                ...
                style: const TextStyle(
                  ...
                  color: Colors.teal, // Safe route highlighted in light green
                ),
              ),
            ),
